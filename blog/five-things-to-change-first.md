...
This is `siteConfig`: your name, tagline, description, GitHub URL, and theme color. It's read by `BaseLayout.astro` for the page title, the meta description, and the Open Graph tags, so it's worth getting right before you worry about anything visual. If you're not using GitHub Sponsors, set `githubSponsorsUrl` to an empty string and the "Support" link in the header and footer disappears on its own — no template conditional to hunt down.
